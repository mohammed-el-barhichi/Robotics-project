...
for idx, node in enumerate (path[1:]):
    distance_totale=robot.distance()
    print(angle)
    node_actuel=node
    node_precedent=path[idx]


    if angle=='up':
        if node_precedent.direction_to_neighbor(node_actuel)=='right':
            tourner_droite()
        elif node_precedent.direction_to_neighbor(node_actuel)=='left':
            tourner_gauche()
        elif node_precedent.direction_to_neighbor(node_actuel)=='down':
            tourner_droite()
            tourner_droite()
    ...
    if angle=='down':
        if node_precedent.direction_to_neighbor(node_actuel)=='right':
            ...
            tourner_droite()
        elif node_precedent.direction_to_neighbor(node_actuel)=='up':
            tourner_droite()
            tourner_droite()

    if angle=='right':
        if node_precedent.direction_to_neighbor(node_actuel)=='down':
            tourner_droite()
        elif node_precedent.direction_to_neighbor(node_actuel)=='up':
            tourner_gauche()
        elif node_precedent.direction_to_neighbor(node_actuel)=='left':
    ...
    if angle=='left':
        if node_precedent.direction_to_neighbor(node_actuel)=='up':
            tourner_droite()
        elif node_precedent.direction_to_neighbor(node_actuel)=='down':
            tourner_gauche()
        elif node_precedent.direction_to_neighbor(node_actuel)=='right':
            tourner_droite()
            tourner_droite()
    ...
    distance_parcourue=robot.distance()-distance_totale
    print(distance_parcourue)
    print(node_precedent.distance_to_neighbor(node_actuel))
    while distance_parcourue < node_precedent.distance_to_neighbor(node_actuel):
        if line_sensor.reflection() - threshold < 10 :
            PROPORTIONAL_GAIN = 0.5
            DRIVE_SPEED=100
        else :
    ...
    angle=node_precedent.direction_to_neighbor(node_actuel)
